const { EmbedBuilder, ActionRowBuilder, UserSelectMenuBuilder } = require("discord.js");
const { findEventById } = require("../../../services/activityTracker.service");

module.exports = async (client, interaction) => {
  const userId = interaction.user.id;
  const guildId = interaction.guildId;
  const buttonKey = interaction.customId;
  const eventId = buttonKey.replace(/^editEventHosts_/, ""); 
  // 提取eventId
  const event = await findEventById(eventId);
  if (!event) {
    return interaction.reply({
      content: "❌ 找不到活動。",
      ephemeral: true,
    });
  }

  // 創建用戶選擇器來修改主持人
  const hostSelectMenu = new UserSelectMenuBuilder()
    .setCustomId(`updateEventHosts_${eventId}`)
    .setPlaceholder('請選擇主持人～')
    .setMinValues(1)
    .setMaxValues(25);
  
  // 如果有現有主持人，預先勾選
  if (event.hostIds && event.hostIds.length > 0) {
    hostSelectMenu.setDefaultUsers(event.hostIds);
  }

  const row = new ActionRowBuilder().addComponents(hostSelectMenu);

  return interaction.reply({
    content: '請選擇主持人',
    components: [row],
    ephemeral: true
  });
};
